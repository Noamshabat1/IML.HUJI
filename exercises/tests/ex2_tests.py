...
def test_preprocess_bad_rows_negative_values():
    ...
    assert not (X[['bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors', 'waterfront', 'sqft_above',
                           "sqft_basement", "yr_built"]].astype(float) < 0).any().any()
...
def test_preprocess_when_bad_values_in_irrelevant_columns_should_keep_row():
    # Arrange
    X, y = create_dummy_house_dataframe(
        # id,      date,            price, bed, bat,   living, lot,   floors, waterfront,view,condition,grade,sqft_above,sqft_basement,yr_built,  yr_renovated, zipcode,  lat,        long,  sqft_living15, sqft_lot15
...
def test_split(X: pd.DataFrame, y: pd.Series, train_proportion: float = .75):
    ...
    print("testing split: ")
    print("train_x", train_x.shape, "expected: ", np.ceil(proportion * X.shape[0]))
...
def test_preprocess_when_only_X_provided_should_succeed():
    ...
    X, y = create_dummy_house_dataframe(
        [["7129300520", "20141013T000000", 221900, 3, 1, 500, 5650, "1", 0, 0, 3, 7, 1180, 0, 1955, 0, "98178", 47.5112, -122.257, 1340, 5650],
         ...
         ["5631500400", "20150225T000000", 180000, 2, 1, 770, 10000, "1", 0, 0, 3, 6, 770, 0, 1933, 0, "98028", 47.7379, -122.233, 2720, 8062]])
    # Act
    X = preprocess_data(X.assign(price=y))[0]  # Call should succeed and return a dataframe
